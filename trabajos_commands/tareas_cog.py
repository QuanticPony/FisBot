...
class tareas_commands(
    ...
    async def Añade(self, context): #funcion para añadir un trabajo nuevo, si eres admin
        ...
        asignatura = await client.wait_for('message', check=CompruebaAutor(ctx,original)) #No se como asegurar que le he pasado como parámetro a la función ctx, osea el contexto de quien lo manda
        i=0
        ...
        else: #cuando tenemos el nombre de la asignatura bien recolectamos los datos necesarios para definir un trabajo nuevo
            if not context.author.dm_channel:
                await context.author.create_dm()
